...
alloc   =  -(16 + 16 + size) & -16                  // pre-increment value for the stack pointer (used in main "prologue")
...
main:
    ...
    add     base, base, ia_s                        // array base location calculated by adding its offset to fp
...
loop1:                                              // first for-loop (to generate random array)
    ...
    ldr    randomNum, [base, index, SXTW 2]         // load random number from stack (to print below) 
...
loop3:                                              // inner for-loop in loop2
    ...
    mov     min, j_index                            // min = j
    str     j_index, [fp, min_s]                    // store j on stack in min's position
...
loop3Test:                                          // inner for-loop test
    ...
    ldr     index, [fp, i_s]                        // load current index from stack
    ldr     w27, [base, index ,SXTW 2]              // store v[i] in w27
    
    // swap elements
    mov     temp, w27                               // temp = v[min] 
    mov     w27, vMin                               // v[min] = v[i]
    mov     vMin, temp                              // v[i] = temp

    str     vMin, [base, min, SXTW 2]               // store current min in vMin
    str     w27, [base, index, SXTW 2]              // store v[i] in w27

    // print string literal to standard output 
    adrp    x0, element
    add     x0, x0, :lo12:element
    mov     w1, index                               // first argument for printf is the current index
    ldr     w27, [base, index ,SXTW 2]              // load v[i] using current index
    mov     w2, w27                                 // second argument fro printf is v[i]
    bl      printf                                  // branch and link printf
